...
class Paralelo
{
    ...
    public function obtenerParalelos($id_periodo_lectivo)
    {
        $this->db->query("SELECT pa.id_paralelo, 
                           ...
                           WHERE pl.id_periodo_lectivo = pa.id_periodo_lectivo 
                             ...
                             AND jo.id_jornada = pa.jornada_id
    ...
    {
        $this->db->query("SELECT * FROM sw_paralelo WHERE curso_subnivel_id = $curso_subnivel_id AND jornada_id = $jornada_id AND id_periodo_lectivo = $periodo_lectivo_id AND pa_nombre = '$pa_nombre'");
    ...
    {
        $this->db->query("SELECT MAX(pa_orden) AS max_orden FROM sw_paralelo WHERE curso_subnivel_id = $datos[curso_subnivel_id] AND jornada_id = $datos[jornada_id] AND id_periodo_lectivo = $datos[periodo_lectivo_id]");
        $registro = $this->db->registro();

        $max_orden = empty($registro) ? 1 : $registro->max_orden + 1;

        $this->db->query("INSERT INTO sw_paralelo SET curso_subnivel_id = :curso_subnivel_id, jornada_id = :jornada_id, id_periodo_lectivo = :periodo_lectivo_id, pa_nombre = :pa_nombre, pa_orden = :pa_orden");
